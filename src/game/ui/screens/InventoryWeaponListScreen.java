...
public class InventoryWeaponListScreen extends MenuScreen
{
    // ---------------- DATA MEMBERS ----------------
    // ---------------- CONSTRUCTORS ----------------

    public InventoryWeaponListScreen(GameState gameState)
    {
        super("INVENTORY WEAPONS", buildChoices(gameState));
    }
    ...
    private static MenuChoice[] buildChoices(GameState gameState)
    {
        ...
        for (int i = 0; i < numberOfWeapons; i++)
        {
            ...
            choices[i] = new MenuChoice(inventoryWeapon.getName(), ScreenType.INVENTORY_WEAPON,
                () -> gameState.setSelectedWeapon(inventoryWeapon));
        }
        choices[numberOfWeapons] = new MenuChoice("(BACK)", ScreenType.INVENTORY);
        return choices;
    }
