...
public class WithdrawJunkScreen extends MenuScreen
{
    // ---------------- DATA MEMBERS ----------------
    // ---------------- CONSTRUCTORS ----------------

    public WithdrawJunkScreen(GameState gameState)
    {
    ...
    private static MenuChoice[] buildChoices(GameState gameState)
    {
        ...
        if (selectedJunk != null)
            ...
            {
                new MenuChoice("MOVE TO INVENTORY", ScreenType.WITHDRAW_JUNK, () ->
                {
                    if (gameState.takeFromStash(selectedJunk))
                        gameState.setSelectedJunk(null);
                }),
                new MenuChoice("(BACK)", ScreenType.WITHDRAW_JUNK_LIST)
            };
        else
            return new MenuChoice[]
            {
